<script lang="ts">
	...
	const trial = $derived($trials.find((it) => it.id === device?.parentId));
	...
	const actionButtonLink = $derived(
		$page.data.connected && device?.status === 'unactive'
			? `${data.baseUrl}/devices/${device?.id}/activation?connected=true&advanced=${$userMode === 'advanced'}`
			: `${data.baseUrl}/devices/pairing?deviceId=${device?.id}`
	);
